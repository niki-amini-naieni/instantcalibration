...
def volumetric_rendering(
    ...
    if compute_extras:
        ...
        if white_background:
            normals_map = normals_map + (1.0 - acc[Ellipsis, None])
        rendering["normals"] = normals_map

        expectation = lambda x: (weights * x).sum(axis=-1) / acc
        expectation_expand = (
            lambda x: (weights[Ellipsis, None] * x).sum(axis=-2) / acc[Ellipsis, None]
        )
        expectation_save = lambda x: (weights * x).sum(-1) / jnp.clip(acc, eps)

        rendering["distance_mean"] = jnp.clip(
            jnp.nan_to_num(expectation(t_mids), jnp.inf), t_vals[:, 0], t_vals[:, -1]
        )
        rendering["distance_mean_save"] = jnp.clip(
            expectation_save(t_mids), t_vals[:, 0], t_vals[:, -1]
        )
        ...
        distance_percentiles = jax.vmap(
            functools.partial(math.weighted_percentile, ps=ps, assume_sorted=True), 0
        )(t_mids, weights)
        ...
        ray_entropy = (
            -weights
            / delta
            * jnp.log(jnp.clip(weights / jnp.clip(delta, eps), eps, 1 - eps))
        )
