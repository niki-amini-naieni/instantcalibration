...
def convolve2d(z, f):
    return jsp.signal.convolve2d(z, f, mode="same", precision=jax.lax.Precision.HIGHEST)
...
def visualize_cmap(
    ...
    if colormap:
        ...
        colorized = value
    if white_background:
        colorized = colorized + (1.0 - weight[Ellipsis, None])
...
def visualize_coord_fix(coords, acc, percentile=99.0):
    ...
    max_val = jnp.maximum(
        1,
        math.weighted_percentile(jnp.max(jnp.abs(coords_fix), axis=2), acc, percentile),
    )
    ...
    return matte(
        jnp.where(mask, (coords_fix_unit + 1) / 2, 1 - jnp.abs(coords_fix_unit)), acc
    )


def visualize_coord_norm(coords, acc, percentile=99.0):
...
def maximize_saturation(rgb):
    ...
    rgb_scaled = pix.hsv_to_rgb(
        jnp.stack(
            [hsv[Ellipsis, 0], scaling * hsv[Ellipsis, 1], hsv[Ellipsis, 2]], axis=-1
        )
    )
    return rgb_scaled
...
def visualize_rays(
    ...
    vis = (
        vis_rgb * vis_alpha[Ellipsis, None]
        + (bg_color * (1 - vis_alpha))[Ellipsis, None]
    )
...
def visualize_suite(rendering, rays, config):
    ...
    rgb = rendering["rgb"]
    acc = rendering["acc"]
    normals = rendering["normals"] / 2.0 + 0.5
    ...
    distance_p95 = rendering[
        "distance_percentile_95"
