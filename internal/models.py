    ...
    def __call__(
        ...
        for i_level in range(self.num_levels):
            ...
            if self.use_viewdirs:
                (rgb, density, normals, gamma, feature_dist) = mlp(
                    rng, samples, rays.viewdirs
                )
            else:
                (rgb, density, normals, gamma, feature_dist) = mlp(rng, samples, None)

            # Volumetric rendering.
            weights, _, trans, delta = mip.compute_alpha_weights(
                density, t_vals, rays.directions
            )

            pi = weights / (jnp.sum(weights, axis=1, keepdims=True) + 1e-12)

            if train and i_level == 0:
                ## flipped reflection ray generation
                # compute the flipped reflection ray directions
                normals_to_use = (weights[Ellipsis, None] * normals).sum(axis=-2)
                flip_directions = (
                    2.0
                    * jnp.sum(normals_to_use * rays.directions, axis=-1, keepdims=True)
                    * normals_to_use
                    - rays.directions
                )
                flip_viewdirs = flip_directions / jnp.linalg.norm(
                    flip_directions, axis=-1, keepdims=True
                )

                # find the hitting point, i.e., estimated object surface
                t_mids = 0.5 * (t_vals[Ellipsis, :-1] + t_vals[Ellipsis, 1:])
                hitting_point = t_mids[
                    jnp.arange(weights.shape[0]), jnp.argmax(weights, axis=-1)
                ][Ellipsis, None]

                # set the origin of the flipped reflection ray
                flip_origins = (rays.origins + hitting_point * rays.directions) - (
                    hitting_point * flip_directions
                )

                # generate the mask filtering the ineffective flipped reflection rays
                cos = (-rays.viewdirs * normals_to_use).sum(axis=-1, keepdims=True)
                cos = jnp.where(jnp.isfinite(cos), cos, -1.0)
                if self.config.llff_flip_mask:
                    flip_mask = jnp.where(jnp.arccos(cos) < jnp.pi / 6.0, 1.0, 0.0)
                else:
                    flip_mask = jnp.where(cos <= 0.0, 0.0, 1.0)

                t_vals_flip, samples_flip = mip.sample_along_rays(
                    key,
                    flip_origins,
                    flip_directions,
                    rays.radii,
                ...
                (rgb_flip, density_flip, _, gamma_flip, feature_dist_flip) = mlp(
                    rng, samples_flip, flip_viewdirs
                )
                weights_flip, _, _, _ = mip.compute_alpha_weights(
                    density_flip, t_vals_flip, flip_directions
                )
                pi_flip = weights_flip / (
                    jnp.sum(weights_flip, axis=1, keepdims=True) + 1e-12
                )
...
def construct_mipnerf(rng, rays, config):
    ...
    init_variables = model.init(
        rng, rng=None, rays=ray, resample_padding=0.0, compute_extras=False
    )
...
class MLP(nn.Module):
    ...
    density_bias: float = -1.0  # The shift added to raw densities pre-activation.
    ...
    def __call__(self, rng, samples, viewdirs=None):
        ...
        if self.disable_normals:
            ...
            (density_flat, x_flat), density_grad_flat = predict_density_and_grad_fn(
                rng, means_flat, covs_flat
            )
        ...
        if viewdirs is not None:
            ...
            feature_dist = nn.softmax(bottleneck, axis=-1)
        ...
        return (rgb, density, normals, gamma, feature_dist)
...
def render_image(render_fn, rays, rng, config):
    ...
    for i_chunk, idx0 in enumerate(idx0s):
        ...
        chunk_renderings = [
            {k: utils.unshard(v[0], padding) for k, v in r.items()}
    ...
    for k in chunks[0]:
        if isinstance(chunks[0][k], list):
            ...
            rendering[k] = rendering[k].reshape(
                (height, width) + chunks[0][k].shape[1:]
            )
    ...
    if keys:
        ray_idx = random.permutation(random.PRNGKey(0), rendering[keys[0]][0].shape[0])[
            : config.vis_num_rays
        ]
    for k in keys:
